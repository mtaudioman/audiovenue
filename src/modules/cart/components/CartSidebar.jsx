    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 shadow-2xl transform transition-transform duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
          ...
          <button onClick={closeCart} className="p-2 hover:bg-zinc-100 rounded-lg transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {cart?.items.length > 0 && (
          <div className="px-6 py-4 border-t border-zinc-100 space-y-3">
            ...
            <button
              onClick={closeCart}
              className="block w-full text-center text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
            >
              Continue Shopping
            </button>
          </div>
